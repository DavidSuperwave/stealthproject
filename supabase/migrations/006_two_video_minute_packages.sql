...
begin
  if exists (select 1 from public.credit_packages where name = '25 minutos') then
    ...
    set
      ...
      features = '{"25 minutos de video","Acceso a guiones","Flujo guiado de campañas"}',
      is_best_value = false,
    ...
    values
      ('25 minutos', 150000, 125, 25, '{"25 minutos de video","Acceso a guiones","Flujo guiado de campañas"}', false, true, true, 10);
  end if;
  ...
  if exists (select 1 from public.credit_packages where name = '75 minutos') then
    ...
    set
      ...
      features = '{"75 minutos de video","Acceso a guiones","Planeación de campañas","Revisión prioritaria de flujo"}',
      is_best_value = true,
      includes_scripts = true,
      active = true,
      sort_order = 20,
    ...
    insert into public.credit_packages
      (name, price_cents_mxn, credits, minutes_equivalent, features, is_best_value, includes_scripts, active, sort_order)
    values
      ('75 minutos', 400000, 375, 75, '{"75 minutos de video","Acceso a guiones","Planeación de campañas","Revisión prioritaria de flujo"}', true, true, true, 20);
  end if;
end $$;
