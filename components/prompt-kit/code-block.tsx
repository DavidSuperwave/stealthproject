...
function CodeBlock({ children, className, ...props }: CodeBlockProps) {
  return (
    <div className={cn('not-prose overflow-hidden rounded-lg border border-border bg-bg-elevated', className)} {...props}>
      {children}
    </div>
  )
}
...
function CodeBlockGroup({ children, className, ...props }: CodeBlockGroupProps) {
  return (
    <div className={cn('flex items-center justify-between border-b border-border px-4 py-2 text-xs text-text-muted', className)} {...props}>
      {children}
    </div>
  )
}
